...
const Trader = function (config) {
  ...
  (async () => {
    try {
      ...
      retFlag = true;
      console.log(
        'error loading markets : ' + this.name + '-' + this.exchangeName,
        e
      );
    }
    retFlag = true;
...
Trader.prototype.processError = function (funcName, error) {
  ...
  if (error instanceof ccxtError.ExchangeError) {
    console.log(
      `[ccxt-${this.exchangeName}] (${funcName}) returned an error, retrying: ${error.message}`
    );
    return new Errors.RetryError(
      '[ccxt-' + this.exchangeName + '] ' + error.message
    );
  } else if (error instanceof ccxtError.NotSupported) {
    console.log(
      `[ccxt-${this.exchangeName}] (${funcName}) returned an irrecoverable error: ${error.message}`
    );
...
Trader.prototype.handleResponse = function (funcName, callback) {
  return (error, body) => {
    if (!error) {
      if (_.isEmpty(body)) error = new Error('NO DATA WAS RETURNED');
    }
...
Trader.prototype.getPortfolio = function (callback) {
  ...
  var processResult = function (err, data) {
    ...
    if (!_.isNumber(assetAmount) || _.isNaN(assetAmount)) {
      console.log(
        `[ccxt-${this.exchangeName}] did not return portfolio for ${this.asset}, assuming 0.`
      );
      assetAmount = 0;
    }
    ...
    if (!_.isNumber(currencyAmount) || _.isNaN(currencyAmount)) {
      console.log(
        `[ccxt-${this.exchangeName}] did not return portfolio for ${this.currency}, assuming 0.`
      );
      currencyAmount = 0;
    }
    ...
    console.log(
      '[ccxt-' + this.exchangeName + '] (getPortfolio) portfolio:',
      portfolio
    );
  ...
  let handler = (cb) =>
    processAttempt(this.ccxt, this.handleResponse('getPortfolio', cb));
  retry(null, _.bind(handler, this), _.bind(processResult, this));
};
...
Trader.prototype.getTicker = function (callback) {
  ...
  let handler = (cb) =>
    processAttempt(this.ccxt, this.pair, this.handleResponse('getTicker', cb));
  retry(null, _.bind(handler, this), _.bind(processResult, this));
};
...
Trader.prototype.buy = function (amount, price, callback) {
  ...
  var processResult = function (err, data) {
    ...
    var txid = data['id'];
    console.log(
      '[ccxt-' + this.exchangeName + '] (buy) added order with txid:',
      txid
    );
...
Trader.prototype.sell = function (amount, price, callback) {
  ...
  var processResult = function (err, data) {
    ...
    var txid = data['id'];
    if (_.isUndefined(txid)) txid = 0; //Order id is undefined, assuming order is already filled. See https://github.com/ccxt/ccxt/issues/660
    console.log(
      '[ccxt-' + this.exchangeName + '] (sell) added order with txid:',
      txid
    );
...
Trader.prototype.getOrder = function (order, callback) {
    ...
    (async () => {
      try {
        ...
        if (e instanceof ccxtError.OrderNotCached)
          cb(undefined, { timestamp: 0, price: 0, amount: 0 });
        //If no order found, assuming already cancelled or filled.
        else cb(e);
      }
    })();
  };
  ...
  let handler = (cb) =>
    processAttempt(
      ...
      this.pair,
      order,
      this.handleResponse('getOrder', cb)
    );
  retry(null, _.bind(handler, this), _.bind(processResult, this));
};
  ...
  var processAttempt = function (ccxt, order, pair, cb) {
    (async () => {
      try {
        ...
        console.log(e);
        if (e instanceof ccxtError.OrderNotCached)
          cb(undefined, { status: 'closed' });
        //If no order found, then order is cancelled or filled.
        else {
          cb(e);
        }
      }
...
Trader.prototype.cancelOrder = function (order, callback) {
  var processAttempt = function (ccxt, order, cb) {
    (async () => {
      try {
        ...
        console.log(e);
        if (e instanceof ccxtError.OrderNotFound) cb(undefined, true, data);
        //If no order found, then order is cancelled or filled.
        else {
  ...
  let handler = (cb) =>
    processAttempt(this.ccxt, order, this.handleResponse('cancelOrder', cb));
