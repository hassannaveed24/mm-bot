...
const Trader = function (config) {
  _.bindAll(this, ['roundAmount', 'roundPrice', 'isValidPrice', 'isValidLot']);
  ...
  return (error, body) => {
    ...
    if (error) {
      ...
      if (
        ...
        error.message.includes('UNKNOWN_ORDER')
      ) {
      ...
      if (
        ...
        console.log(
          new Date(),
          'Binance doesnt know this order, retrying up to 10 times..'
        );
        error.retry = 10;
      }
      ...
      if (
        funcName === 'addOrder' &&
        error.message.includes('Account has insufficient balance')
      ) {
...
Trader.prototype.getPortfolio = function (callback) {
  const setBalance = (err, data) => {
    ...
    let assetAmount = parseFloat(_.find(data.balances, findAsset));

    const findCurrency = (item) => item.asset === this.currency;
    let currencyAmount = parseFloat(_.find(data.balances, findCurrency));
...
Trader.prototype.getFee = function (callback) {
  ...
  const fetch = (cb) =>
    this.binance.account({}, this.handleResponse('getFee', cb));
...
Trader.prototype.getOpenOrders = function (callback) {
  ...
  const handler = (cb) =>
    this.binance.openOrders(reqData, this.handleResponse('getOpenOrders', cb));
...
Trader.prototype.getOrder = function (order, callback) {
  const get = (err, data) => {
    ...
    if (!trades.length) {
      console.log('cannot find trades!', {
        order,
        list: data.map((t) => t.orderId).reverse(),
      });
      ...
      this.binance.queryOrder(reqData, (err, resp) => {
        console.log('couldnt find any trade for order, here is order:', {
          err,
          resp,
        });
    ...
    _.each(trades, (trade) => {
      ...
      price =
        (price * amount + +trade.price * trade.qty) / (+trade.qty + amount);
      amount += +trade.qty;

      if (fees[trade.commissionAsset])
        fees[trade.commissionAsset] += +trade.commission;
      else fees[trade.commissionAsset] = +trade.commission;
    });
  ...
  const handler = (cb) =>
    this.binance.myTrades(reqData, this.handleResponse('getOrder', cb));
  retry(undefined, handler, get);
};
