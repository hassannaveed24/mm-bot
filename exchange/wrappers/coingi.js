...
var Errors = require('../exchangeErrors');
...
var Trader = function (config) {
  ...
  this.coingi = new Coingi(this.key, this.secret, {
...
Trader.prototype.processError = function (funcName, error) {
  ...
  if (!error.message.match(recoverableErrors)) {
    log.error(
      `[coingi.js] (${funcName}) returned an irrecoverable error: ${error.message}`
    );
    return new Errors.AbortError('[coingi.js] ' + error.message);
  }

  log.debug(
    `[coingi.js] (${funcName}) returned an error, retrying: ${error.message}`
  );
...
Trader.prototype.handleResponse = function (funcName, callback) {
  return (error, body) => {
    if (!error) {
      ...
      else if (!_.isEmpty(body.error)) error = new Error(body.error);
    }
...
Trader.prototype.getTrades = function (since, callback, ascending) {
  ...
  var processResults = function (err, trades) {
    ...
    if (ascending) callback(undefined, parsedTrades);
...
Trader.prototype.getPortfolio = function (callback) {
  var setBalance = function (err, data) {
    ...
    log.debug(
      '[coingi.js] entering "setBalance" callback after coingi-api call, data:',
      data
    );
...
Trader.prototype.addOrder = function (tradeType, amount, price, callback) {
  ...
  let handler = (cb) =>
    this.coingi.api('add-order', reqData, this.handleResponse('addOrder', cb));
  util.retryCustom(
    retryCritical,
    _.bind(handler, this),
...
Trader.prototype.getOrder = function (orderId, callback) {
  var getOrder = function (err, order) {
    ...
    if (order !== null) {
      ...
      log.error(
        "Error! Order ID '" +
          orderId +
          "' couldn't be found in the result of get order!"
      );
    }
  };
  ...
  let handler = (cb) =>
    this.coingi.api('get-order', reqData, this.handleResponse('getOrder', cb));
  util.retryCustom(
    retryCritical,
    _.bind(handler, this),
    _.bind(getOrder, this)
  );
};
